...
const Dashboard = () => {
  ...
  const courseProgress = [
    ...
    { label: "Advanced Japanese Grammar", percent: 86, link: "/" }, // Long label
  ];
        ...
        {hasCourses && (
          <div className="grid grid-cols-2 gap-4 w-full max-w-3xl px-2">
            {courseProgress.map((course, idx) => {
              const content = (
                <div
                  className="relative rounded-xl overflow-hidden border border-pink-600 dark:border-[#F66538] bg-gray-200 dark:bg-[#3B3E40] h-28 w-full"
                >
                  <Image
                    src={`/course-thumbnails/course${(idx % 3) + 1}.jpg`}
                    alt={course.label}
                    fill
