...
export default function VocabCard({ data, onSwipe }) {
  ...
  const hiragana = wanakana.toHiragana(data.furigana); // ← Convert here
      ...
      >
        ...
        >
          ...
          <div className="h-auto w-full text-center">
            <div className="text-7xl font-bold">{data.word}</div>
            <div className="text-xl mt-2">{hiragana}</div>
            <div>{data.furigana}</div>
          </div>
        </div>
        ...
        >
          ...
          <div>
            <strong>Kanji Used:</strong> {data.kanji_type}
          </div>
        </div>
      </div>
    </div>
