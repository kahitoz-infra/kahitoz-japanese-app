...
export default function SetBlock({ setNo, status = "not attempted", score = null, lastAttempted = null, type, quiz_number }) {
  ...
  return (
    <div className={`p-4 border rounded-xl shadow-sm ${colorClass} transition-transform duration-200 w-full relative`}>
      ...
      <div className="mt-4 flex justify-end">
        <button
          onClick={() =>
            router.push(
              `/Quiz?setNo=${setNo}&type=${type}&quiz_number=${quiz_number}`
            )
          }
