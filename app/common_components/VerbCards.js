...
export default function VerbCard({ data, onSwipe }) {
  ...
  const hiragana = wanakana.toRomaji(data.grammar); 
      ...
      >
        ...
        >
          ...
          <div className="h-auto w-full text-center">
            <div className="text-7xl font-bold">{data.verb}</div>
            <div className="text-xl mt-2">{data.furigana}</div>
            <div>{data.romaji}</div>
          </div>
        </div>
